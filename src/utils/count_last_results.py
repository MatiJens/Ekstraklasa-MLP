...
def count_last_results(matches_df):
    ...
    matches_df_home_copy = matches_df[['date', 'home', 'result']].copy()
    ...
    matches_df_copy = matches_df_copy.sort_values(by=['team_id', 'date']).reset_index(drop=True)
    ...
    matches_df_copy = matches_df_copy.sort_values(by='date')

    # Creating new columns that say last_results of home and away team in matchId
    matches_df_copy = matches_df_copy.set_index(['date', 'is_home'])['last_results'].unstack()
